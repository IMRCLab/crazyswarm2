...
In contrast, Crazyswarm2 provides a focused set of essential tools for simulation, visualization, and control to allow a wide variety of applications, including applications that require significant low-level firmware changes.
...
Its main components include the Crazyflie Server, the simulation framework, and a separate Python library that simplifies the command handling through ROS 2.
...
This configuration YAML file also contains all the logging and parameters that need to be initialized within the Crazyflie ecosystem.
...
Additionally, the server converts any control topics to their Crazyflie framework equivalent through the commander structure, which is used for both the pitch/roll/yaw and velocity/position commands for control in real time.
...
(1) is a C++-based library that was developed alongside the original Crazyswarm project and reimplemented for Crazyswarm2.
...
The simulation backend acts as a gateway to the hybrid software-in-the-loop (SITL) simulation. The hybrid SITL consists of wrappers that convert the original Crazyflie's C-based firmware into callable Python functions, which can be called from the ROS 2 node.
...
The main purpose of this library is to provide a simplified interface for users to control their Crazyflies as a layer above the full ROS 2 interface.
...
- Integration with ROS 2 [@crazysim]
...
In the future, we plan to add a Rust backend as the default option, which will allow us to reuse more of the official vendor's software stack without sacrificing performance for larger teams. We are also planning to improve the swarm management tools to better support distributed operation use cases.
